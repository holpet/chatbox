...
async function logoutAndContinue(logout, login) {
    $(logout).on('click', async function() {
        if ($(logout).attr('aria-expanded') === 'true') {
            if ($(login).attr('aria-expanded') === 'true') document.getElementById('reg-user-choose').click();
            $('#anon-check').fadeIn(200).text('check_box');
            await logoutUser();
        }
        else {
            $('#anon-check').fadeIn(200).text('check_box_outline_blank');
        }
    });
}
...
async function loginAndContinue(login, logout) {
    $(login).on('click', async function() {
        if ($(login).attr('aria-expanded') === 'true') {
            if ($(logout).attr('aria-expanded') === 'true') document.getElementById('anon-user-choose').click();
            $('#reg-check').fadeIn(200).text('check_box');
            await loginUser();
            $('#reg-user-continue').attr('href', '/' + exampleUsers[num].username.toLowerCase());
        }
        else {
            $('#reg-check').fadeIn(200).text('check_box_outline_blank');
        }
    });
}
